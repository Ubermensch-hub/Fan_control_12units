...
void HAL_TIM_Base_MspDeInit(TIM_HandleTypeDef *tim_baseHandle)
	...
	HAL_GPIO_DeInit(GPIOA,
		Buf_PWM_IN_5_Pin | Buf_PWM_IN_4_Pin | MC_PWM_OUT_5_Pin
			| MC_PWM_OUT_4_Pin);
...
int count_connected_devices()
    {
    int count = 0;
    if (Buf_PWM_IN_5.is_on)
	count++;
    if (Buf_PWM_IN_4.is_on)
	count++;
    if (BUF_PWM_IN_3.is_on)
	count++;
    if (BUF_PWM_IN_2.is_on)
	count++;
    if (BUF_PWM_IN_1.is_on)
	count++;
    if (BUF_PWM_IN_0.is_on)
	count++;
    return count;
    }
...
void set_PWM(int num_devices)
	...
	{
	MC_PWM_OUT_0_PW = MC_PWM_OUT_1_PW = MC_PWM_OUT_2_PW = MC_PWM_OUT_3_PW = MC_PWM_OUT_4_PW = MC_PWM_OUT_5_PW = BUF_PWM_IN_?.pulse_width;
